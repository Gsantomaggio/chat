...
class TestMessageHandler(unittest.TestCase):
    ...
    def test_read_message_send(self):
        ...
        with patch(
            "source.message_handler.read_string",
            side_effect=[("Hello", 18), ("user2", 25), ("user1", 32)],
        ):
            with patch(
                "source.message_handler.read_timestamp",
                return_value=(int(time.time()), 40),
            ):
    ...
    def test_send_message(self):
        message = Message(1, "Hello", "user1", "user2", int(time.time()))
